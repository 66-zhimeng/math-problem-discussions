...
> **内核代码已迁到独立仓库 [layout-kernel](https://github.com/66-zhimeng/layout-kernel)**（2026-09-18，保留完整提交历史，可 pip 安装，约束全部由调用方配置）。
> 本目录的代码停止维护，只作为研究记录保留：数学模型、各轮实验脚本与结论（含失败的）。今后内核的修改都提交到新仓库。
